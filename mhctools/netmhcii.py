...
from .parsing import parse_netmhcii_stdout

logger = logging.getLogger(__name__)

class NetMHCII(BaseCommandlinePredictor):
    def __init__(
            self,
            alleles,
            default_peptide_lengths=[15],
            program_name="netMHCII",
            process_limit=-1):
        BaseCommandlinePredictor.__init__(
            ...
            parse_output_fn=parse_netmhcii_stdout,
            supported_alleles_flag="-list",
            input_file_flag="-f",
            allele_flag="-a",
            peptide_mode_flags=["-inptype", "1"],
            length_flag="-l",
            #tempdir_flag="-tdir", # netMHCII-2.3 has a bug: it fails on trying to create this directory when it exists
